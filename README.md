...
API for fruit data retrieval and addition.

Learn more about Fruityvice at [fruityvice.com](https://fruityvice.com).
...
This SDK exposes the API as a small set of **semantic entities** — Fruit — that you
...
const items = await client.Fruit().list()
```
...
```go
client := sdk.Test()
result, err := client.Fruit(nil).List(
    nil, nil,
)
```
...
$fruits = $client->Fruit()->list();
print_r(array_map(fn($item) => $item->data_get(), $fruits));
