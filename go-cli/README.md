...
boru-driven command-line client **and** interactive REPL for the Fruityvice
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fruityvice-cli)
make build
...
./fruityvice-cli load 1 fruit            # {id:1} shorthand
./fruityvice-cli load '{id:1}' fruit       # explicit match map
./fruityvice-cli update '{name:"x"}' fruit

# 5. Override the API base URL for a single call
FRUITYVICE_BASE=https://api.example.com ./fruityvice-cli list fruit

# 6. No arguments -> interactive REPL
./fruityvice-cli
fruityvice> list fruit
fruityvice> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fruityvice-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fruityvice-cli list fruit
   ```
...
```sh
./fruityvice-cli load 1 fruit          # scalar shorthand for {id:1}
./fruityvice-cli load '{id:1}' fruit     # explicit match map
```
...
```sh
./fruityvice-cli update '{id:1,name:"new"}' fruit
```
...
Run with no arguments to open a REPL (prompt `fruityvice>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `fruit`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fruityvice-cli`. |
...
(`./fruityvice-cli <expr>`) and interactively (the REPL), and expressions compose the
