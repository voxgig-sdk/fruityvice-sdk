// Command Fruityvice-cli is a CLI + REPL for the Fruityvice SDK,
...
const prompt = "fruityvice"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "fruit"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FRUITYVICE_APIKEY (api key), FRUITYVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FRUITYVICE_APIKEY carries the API key and
	// FRUITYVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FRUITYVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FRUITYVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFruityviceSDK(opts)
