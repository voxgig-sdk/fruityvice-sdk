# Fruityvice TypeScript SDK



The TypeScript SDK for the Fruityvice API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const fruit = await client.Fruit().load({ id: 'example_id' })
  console.log(fruit)
} catch (err) {
...
const fruit = await client.Fruit().load({ id: 'test01' })
...
| `tester(testopts?, sdkopts?)` | `FruityviceSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FruityviceSDK.test(testopts?, sdkopts?)` | `FruityviceSDK` | Create a test-mode client. |
...
| `client` | `client(): FruityviceSDK` | Return the parent SDK client. |
...
fruityvice/
├── src/
│   ├── FruityviceSDK.ts        # Main SDK class
