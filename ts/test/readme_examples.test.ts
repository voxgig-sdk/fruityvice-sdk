...
//     (\`new FruityviceSDK(...)\` / \`FruityviceSDK.test(...)\` become
//     \`FruityviceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"fruit":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FruityviceSDK(...)\` and any
// \`FruityviceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FruityviceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FruityviceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
