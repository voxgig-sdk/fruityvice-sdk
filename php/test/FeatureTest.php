...
class FtCtrl extends FruityviceControl
{
...
class FtClient
{
    ...
    private FruityviceUtility $utility;

    public function __construct(FruityviceUtility $utility, array $options)
    {
    ...
    public function get_utility(): FruityviceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FruityviceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FruityviceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FruityviceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FruityviceConfig::make_feature($name);
    ...
    private static function buildUrl(FruityviceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FruityviceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FruityviceResponse($fetched);
        }
    ...
    private function fail(FruityviceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FruityviceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
