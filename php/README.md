...
The PHP SDK for the Fruityvice API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Fruit(null)->list(null, null);
...
```php
[$result, $err] = $client->Fruit(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
```php
// Update
$client->Fruit(null)->update(["id" => $created["id"], "name" => "Example-Renamed"], null);
...
[$result, $err] = $client->Fruityvice(null)->load(
    ["id" => "test01"], null
);
...
```
php/
├── fruityvice_sdk.php          -- Main SDK class
...
The main class (`fruityvice_sdk.php`) exports the SDK class
