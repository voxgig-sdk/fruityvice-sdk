...
module FruityviceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FruityviceResponse.new({})
    ...
    if fetch_err
      ...
      response = FruityviceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FruityviceResponse.new(fetched)
    else
