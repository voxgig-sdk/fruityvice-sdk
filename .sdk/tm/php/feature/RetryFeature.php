...
class FruityviceRetryFeature extends FruityviceBaseFeature
{
    ...
    public function init(FruityviceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FruityviceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FruityviceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FruityviceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
