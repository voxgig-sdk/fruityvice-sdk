...
MCP server exposing the Fruityvice SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `fruityvice_list` | `entity`, optional `query` map | First page of records as JSON |
| `fruityvice_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `fruityvice_list` and
`fruityvice_load` tools become available in new sessions.
