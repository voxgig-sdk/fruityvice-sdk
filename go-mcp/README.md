...
[MCP](https://modelcontextprotocol.io) server exposing the Fruityvice SDK as
two agent tools — `fruityvice_list` and `fruityvice_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fruityvice-mcp)
make build

# 2. Provide credentials via the environment
export FRUITYVICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fruityvice \
  -- /absolute/path/to/fruityvice-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fruityvice-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fruityvice_list: first page of records
{ "entity": "fruit" }
{ "entity": "fruit", "query": { } }

// fruityvice_load: one record by id
{ "entity": "fruit", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fruityvice-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fruityvice \
     -- "$PWD"/dist/*/fruityvice-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fruityvice_list` and `fruityvice_load` tools now appear
   in new sessions. Ask the agent to *"list fruit using fruityvice"*
   and it calls `fruityvice_list` with `{"entity":"fruit"}`.
...
```sh
export FRUITYVICE_APIKEY=sk_live_xxx            # API key
export FRUITYVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fruityvice_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fruityvice_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FRUITYVICE_APIKEY` | API key sent with every request. |
| `FRUITYVICE_BASE` | Optional override of the API base URL. |
...
```sh
./fruityvice-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fruityvice_load","arguments":{"entity":"fruit","query":{"id":1}}}}'
```
