...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Fruit()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/fruityvice-sdk/releases)) or
...
```python
client = FruityviceSDK.test(testopts, sdkopts)
```
...
| `genus` | `str` | Botanical genus of the fruit |
| `id` | `int` | Unique identifier for the fruit |
| `message` | `str` |  |
| `name` | `str` | Name of the fruit |
| `nutritions` | `dict` | Nutritional information per 100 grams |
| `order` | `str` | Botanical order of the fruit |
...
```
py/
├── fruityvice_sdk.py         -- Main SDK module
...
The main module (`fruityvice_sdk`) exports the SDK class.
