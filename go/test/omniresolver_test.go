...
import (
	...
	omni "github.com/voxgig-sdk/fruityvice-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FruityviceError); is {
				out := map[string]any{
					"name":    "FruityviceError",
...
func omniCtx(arg any, client *sdk.FruityviceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FruityviceSDK); is {
			client = live
