...
class FruitEntity
  ...
  def data_set(args)
    if args
      @_data = FruityviceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FruityviceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Fruit, Hash] the loaded Fruit; raises FruityviceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FruityviceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Fruit>, Array] the matching Fruit items; raises FruityviceError on failure
  ...
  #
  # @param reqdata [FruitUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Fruit, Hash] the updated Fruit; raises FruityviceError on failure
