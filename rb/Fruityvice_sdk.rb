...
require_relative 'Fruityvice_types'
...
class FruityviceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FruityviceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FruityviceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FruityviceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FruityviceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FruityviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FruityviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FruityviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FruityviceError.new(
        "#{op}_allow",
        "FruityviceSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FruityviceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FruityviceError.new(
        "graphql_error", "FruityviceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
